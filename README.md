...
- ~Travis-CI.org doesn't support Docker volume mounting (except its Enterprise plan), so you cannot transfer neither source nor ouput files to/from the container. I chose to run all build task inside the Docker container, tell me if you have any better idea :). [Update 1] It seems we can get the job done quicker using Docker volume mount now~. [Update 2] The [docker volume binding feature](https://docs.travis-ci.com/user/enterprise/worker-configuration/#mounting-volumes-across-worker-jobs-on-enterprise) is only available for Enterprise user.
